...
const Button = ({ label, type = 'button', success, iconBefore, iconAfter, outlined, dark, children, className, rounded, onClick }: ButtonLinkProps) => {
    const buttonClass = () => {
        if (dark && outlined) {
    ...
    return (
        <button onClick={onClick} type={type} className={`inline-flex items-center px-5 py-0.5 md:py-2.5 overflow-hidden group relative hover:bg-gradient-to-r hover:ring-2 hover:ring-offset-2 transition-all ease-out duration-300 ${buttonClass()} ${className} ${rounded ? 'rounded-3xl' : 'rounded'}`}>
            ...
            {children ? children : <span className={`relative ${iconAfter || iconBefore ? 'pt-0.5 px-1' : 'py-px'}`}>{label || 'Click Me!'}</span>}
            {iconAfter && iconAfter}
        </button>
